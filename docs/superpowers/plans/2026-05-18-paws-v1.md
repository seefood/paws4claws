...
| `examples/nanoclaw/use-paws/SKILL.md` | Create | Claude Code skill for nanoclaw agents                    |
...
def test_ignores_non_prefixed_vars():
    env = {
        "PAWS_TOKEN_A": "good",
        "PAWS_SECRET": "ignored",  # pragma: allowlist secret
        "OTHER": "also",
    }
...
def test_wrong_token_is_401(base_url):
    status, body = _post(
        f"{base_url}/invoke", {"args": ["sts", "get-caller-identity"]}, token="bad"
    )
    assert status == 401
...
def test_allowed_services_none_permits_any_service():
    ...
    with patch("paws.subprocess.run", return_value=mock):
        status, body = _post(
            f"{url}/invoke", {"args": ["kms", "list-keys"]}, token="test-token-xyz"
        )
...
- Create: `examples/nanoclaw/use-paws/SKILL.md`

- [ ] **Step 1: Create `examples/nanoclaw/use-paws/SKILL.md`**
...
```bash
git add examples/nanoclaw/use-paws/SKILL.md
git commit -m "docs: nanoclaw integration skill for use-paws"
```
